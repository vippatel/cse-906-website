...
// To store when is a tag going to come next.
std::unordered_map <unsigned long long int, std::deque<unsigned long long int> > next_dup_trace;
    ...
    if(X->limit < X->cache_lines) {
		...
		for(auto lines = 0; lines < X->cache_lines; lines++) {
			if(index < next_dup_trace[X->cache_matrix[lines].tag_value][0]) {
				index = next_dup_trace[X->cache_matrix[lines].tag_value][0];
				pos = lines;
			}
		}

		next_dup_trace[X->cache_matrix[pos].tag_value].pop_front();
		auto l3_vic_tag_bits = X->cache_matrix[pos].tag_value;
		X->cache_matrix[pos].tag_value = tag_bits;
		X->cache_matrix[pos].counter = ++(global_lru_counter);
        X->lru_counters[pos] = global_lru_counter;
        X->cache_matrix[pos].dirty = false;
        X->cache_matrix[pos].valid = true;
...
void find_next_duplicates(l3_cache *L3, std::vector<unsigned long long int> miss_trace) {
