...
unsigned long long int get_lru_cache_vic (cache *X, 
	...
	if(min_counter > X->cache_matrix[index][j].counter && X->cache_matrix[index][j].tag_value != INVALID_TAG) {
	...
	X->cache_matrix[index][pos].counter = ++block_level_lru_counter;
...
std::pair <unsigned long long int, bool> insert_into_cache (cache *X, 
	...
	if(!list_free_ways.empty()) { 
		...
		X->cache_matrix[index][free_way].counter = ++block_level_lru_counter;
	...
	for(const auto& trace_item : miss_trace) {
		...
		for(auto ways = 0; ways < L2->cache_assoc; ways++) {
			...
			if(l2_tag_bits == exist_tag) {
				l2_hit = true;
				hit_way = ways;
				L2->cache_matrix[l2_set_index][ways].counter = ++block_level_lru_counter;
				L2->hits++;
		...
		if(l2_hit == false) {
			...
			auto exist_tag = L3->cache_matrix[l3_set_index][ways].tag_value;
				if(l3_tag_bits == exist_tag) {
					l3_hit = true;
					hit_way = ways;
					L3->cache_matrix[l3_set_index][ways].counter = ++block_level_lru_counter;                  					
					L3->hits++;
			...
			std::pair <unsigned long long int, bool> ret_pair = insert_into_cache(L3, l3_set_index, l3_tag_bits);
			...
			if(invalidate_l2) {
				...
				for(auto ways = 0; ways < L2->cache_assoc; ways++) {
					if(L2->cache_matrix[l2_addr_set_index][ways].tag_value == l2_addr_tag_bits) {
						...
						L2->cache_matrix[l2_addr_set_index][ways].counter = INT_MIN;
...
int main (int argc, char* argv[], char* envp[]) {
	...
	std::cout << "[INFO Case-1] L2 Hits : " << L2->hits << ", L2 Miss : " << L2->miss  
